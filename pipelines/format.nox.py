...
def reformat_code(session: nox.Session) -> None:
    ...
    session.run(
        "ruff", "check", "--select", "I", "--fix", *config.PYTHON_REFORMATTING_PATHS
    )
...
def remove_trailing_whitespaces(session: nox.Session, check_only: bool = False) -> None:
    ...
    for raw_path in config.FULL_REFORMATTING_PATHS:
        ...
        dir_total, dir_count = remove_trailing_whitespaces_for_directory(
            pathlib.Path(path), session, check_only
        )
    ...
    call(
        ...
        + (
            "\nTry running 'nox -s reformat-code' to fix them"
...
def remove_trailing_whitespaces_for_file(
    file: str, session: nox.Session, check_only: bool
) -> bool:
    try:
